// src/components/admin/products/PriceManager.tsx
...
interface PriceManagerProps {
  ...
  errors?: {
    ...
    threeByTwo?: { message?: string };
    fiveByThree?: { message?: string };
  };
}
...
export default function PriceManager({ prices, onPriceChange, errors }: PriceManagerProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="mb-4">
        <label htmlFor="price-threeByTwo" className="block text-sm font-medium text-gray-700 mb-1">
          Promoción 3x2 (opcional)
        </label>
        <div className="relative rounded-md shadow-sm">
          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <span className="text-gray-500">$</span>
          ...
          <input
            type="number"
            id="price-threeByTwo"
            value={prices.threeByTwo || ''}
            onChange={handlePriceChange('threeByTwo')}
            step="0.01"
            className="w-full pl-7 p-2 border border-gray-300 rounded-md focus:ring-green-dark focus:border-green-dark"
          />
        </div>
        {errors?.threeByTwo && <p className="text-red-500 text-sm">{errors.threeByTwo.message}</p>}
      </div>

      <div className="mb-4">
        <label htmlFor="price-fiveByThree" className="block text-sm font-medium text-gray-700 mb-1">
          Promoción 5x3 (opcional)
        </label>
        <div className="relative rounded-md shadow-sm">
          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <span className="text-gray-500">$</span>
          ...
          <input
            type="number"
            id="price-fiveByThree"
            value={prices.fiveByThree || ''}
            onChange={handlePriceChange('fiveByThree')}
            step="0.01"
            className="w-full pl-7 p-2 border border-gray-300 rounded-md focus:ring-green-dark focus:border-green-dark"
          />
        </div>
        {errors?.fiveByThree && <p className="text-red-500 text-sm">{errors.fiveByThree.message}</p>}
      </div>
    </div>
  );
}